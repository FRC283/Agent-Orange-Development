...
#Constants
IP = "10.2.83.5"
GREEN_LOWER = numpy.array([40, 0, 205])
GREEN_UPPER = numpy.array([120, 100, 255])
TABLE_NAME = "cv_data"
...
jj, init_image = camera.read()
cv2.imshow("cam", init_image)
cv2.waitKey(0)
    ...
    if is_reading: #Only continue if we're successfully reading the image. (Prevents crashes)
        ...
        if (len(contours) > 1):
            ...
            if c0m['m00'] != 0 and c1m['m00'] != 0: #Avoid division by 0
                ...
                dy = (height - 1)/2 - (cmy)  #Calculate the difference in the y between the center of mass and the center of image. Signage correct
